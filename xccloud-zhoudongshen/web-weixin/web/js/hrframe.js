        ...
        HRFrameFunctions["curry"] = function(_fn) {
            ...
            var rtnFn = function() {
                ...
                if (realLength + arg.length >= argumentsLength) {
                    ...
                    HRFrameValues.caller=_fn._name;
...
$f("df", "ajax.get", function(_url, _data, _fn) {
    ...
    if (_url == null) {
        ...
        if (HRFrameValues.ISTEST && HRFrameValues.TEST[_url]!=undefined) {
            callbackdata = HRFrameValues.TEST[_url].rtn;
...
$f("df", "plugin", function(_path, _param) {
    ...
    var getplugin = $f("ajax.getplugin", fileURL(_path) + "/plugin.html", pluginargs, function(_page) {
        ...
        _pageWithPath = _pageWithPath.replace(plugin_path[2],_path);
...
$f("df","plugin.startTest",function(_path){
    var HRconfig = this;
    $f("ajax.syncget","doc/"+_path+"/doc.yml",null,function(_data){
        HRconfig.ISTEST=true;
        var doc = jsyaml.load(_data);
        for(var item in doc){
            HRconfig.TEST[doc[item].url]=doc[item];
        }
    })();
});

$f("df","plugin.closeTest",function(_bool){
    this.ISTEST=false;
});
